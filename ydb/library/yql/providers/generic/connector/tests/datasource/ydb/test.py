...
from ydb.library.yql.providers.generic.connector.api.common.data_source_pb2 import (
    EDataSourceKind,
)
from ydb.library.yql.providers.generic.connector.tests.utils.settings import Settings
from ydb.library.yql.providers.generic.connector.tests.utils.one_time_waiter import (
    YDB as OneTimeWaiter,
)
from ydb.library.yql.providers.generic.connector.tests.utils.run.runners import (
    runner_types,
    configure_runner,
)
...
settings = Settings.from_env(
    docker_compose_dir=docker_compose_dir, data_source_kinds=[EDataSourceKind.YDB]
)
...
@pytest.mark.parametrize(
    "test_case",
    tc_collection.get("select_positive"),
    ids=tc_collection.ids("select_positive"),
)
...
@pytest.mark.parametrize(
    "test_case",
    tc_collection.get("select_missing_table"),
    ids=tc_collection.ids("select_missing_table"),
)
